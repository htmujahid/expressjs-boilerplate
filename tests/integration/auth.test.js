...
const { User, Token } = require('../../src/database/queries');
...
describe('Auth routes', () => {
    describe('POST /v1/auth/register', () => {
        ...
        test('should return 201 and successfully register user if request data is ok', async () => {
            ...
            const dbUser = await User.getUser(res.body.user._id);
            expect(dbUser).toBeDefined();
            expect(dbUser.password).not.toBe(newUser.password);
            expect(dbUser).toMatchObject({ username: newUser.username, email: newUser.email, emailVerified: null });
        ...
        test('should return 400 error if user is already used', async () => {
            await insertUsers([userOne]);
    ...
    describe('POST /api/v1/auth/login', () => {
        ...
        test('should return 401 error if there are no users with that email', async () => {
            ...
            expect(res.body).toEqual({ code: httpStatus.UNAUTHORIZED, message: 'Invalid Credentials' });
        });
    ...
    describe('POST /api/v1/auth/logout', () => {
        test('should return 204 if refresh token is valid', async () => {
            ...
            const dbRefreshTokenDoc = await Token.getToken(refreshToken, tokenTypes.REFRESH);
    ...
    describe('POST /api/v1/auth/refresh-tokens', () => {
        test('should return 200 and new auth tokens if refresh token is valid', async () => {
            ...
            const dbRefreshTokenDoc = await Token.getToken(res.body.refresh.token, tokenTypes.REFRESH);
            expect(dbRefreshTokenDoc).toMatchObject({ type: tokenTypes.REFRESH, userId: userOne._id, blacklisted: false });

            const dbRefreshTokenCount = await Token.getTokensCount(userOne._id, tokenTypes.REFRESH);
    ...
    describe('POST /api/v1/auth/forgot-password', () => {
        ...
        test('should return 204 and send reset password email to the user', async () => {
            ...
            const dbResetPasswordTokenDoc = await Token.getToken(resetPasswordToken, tokenTypes.RESET_PASSWORD);
    ...
    describe('POST /v1/auth/reset-password', () => {
        test('should return 204 and reset the password', async () => {
            ...
            const dbUser = await User.getUserWithPassword(userOne._id);
            const isPasswordMatch = await bcrypt.compare('password2', dbUser.password);
            expect(isPasswordMatch).toBe(true);

            const dbResetPasswordTokenCount = await Token.getTokensCount(userOne._id, tokenTypes.RESET_PASSWORD);
    ...
    describe('POST /v1/auth/send-verification-email', () => {
        ...
        test('should return 204 and send verification email to the user', async () => {
            ...
            const dbVerifyEmailToken = await Token.getToken(verifyEmailToken, tokenTypes.VERIFY_EMAIL);
    ...
    describe('POST /v1/auth/verify-email', () => {
        test('should return 204 and verify the email', async () => {
            ...
            const dbUser = await User.getUser(userOne._id);

            expect(dbUser.emailVerified).toBeDefined();

            const dbVerifyEmailToken = await Token.getTokensCount(userOne._id, tokenTypes.VERIFY_EMAIL);
